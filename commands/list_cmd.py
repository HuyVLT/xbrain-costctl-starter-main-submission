...
def run(args):
    ...
    if args.missing_tag:
        filters.extend([f"!{k}" for k in args.missing_tag])
        
    filter_str = " ".join(filters)
    if filter_str:
        header_title = f"{args.type.upper()} {filter_str}"
    else:
        header_title = f"{args.type.upper()}"
        
    print(f"{header_title} — {len(rows)} found:")
    print("-" * 78)
    for r in rows:
        tags_str = ", ".join(f"{k}={v}" for k, v in r[3].items())
        print(f"  {r[0]:<25} {r[1]:<14} {r[2]:<13} {tags_str}")
