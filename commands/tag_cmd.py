...
def _tag_rds(rid, tags):
    ...
    db_info = rds.describe_db_instances(DBInstanceIdentifier=rid)
    arn = db_info["DBInstances"][0]["DBInstanceArn"]
...
def _tag_s3(rid, tags):
    ...
    existing_tags = []
    try:
        tag_resp = s3.get_bucket_tagging(Bucket=rid)
