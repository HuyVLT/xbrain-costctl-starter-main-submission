    ...
    try:
        ...
        for result in response.get("ResultsByTime", []):
            for group in result.get("Groups", []):
                ...
                services_cost[service_name] += amount
                total_cost += amount
                
        sorted_services = sorted(services_cost.items(), key=lambda item: item[1], reverse=True)
        
        print(f"Cost for {args.tag} over last {args.days} days ({start.strftime('%Y-%m-%d')} → {(end - timedelta(days=1)).strftime('%Y-%m-%d')}):")
        print("-" * 60)
        for svc, cost in sorted_services:
            print(f"  {svc:<45} ${cost:>8.2f}")
        print("-" * 60)
