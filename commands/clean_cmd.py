...
def _find_targets(tag_key, tag_val):
    ...
    # Volume targets
    volume_targets = []
    v_paginator = ec2.get_paginator("describe_volumes")
    for page in v_paginator.paginate():
        for vol in page.get("Volumes", []):
            tags = {t["Key"]: t["Value"] for t in vol.get("Tags", [])}
            if tags.get(tag_key) == tag_val:
                if vol["State"] == "available":
                    volume_targets.append(vol["VolumeId"])
                    
    return {"ec2": ec2_targets, "volume": volume_targets}
...
def run(args):
    ...
    print(f"Found {ec2_count} EC2 instance(s) and {vol_count} volume(s) matching {args.tag}")
    
    if not args.apply:
        print(f"Would terminate instances: {targets['ec2']}")
        print(f"Would delete volumes: {targets['volume']}")
    ...
    if targets["ec2"]:
        ...
        for eid in targets["ec2"]:
            print(f"Terminated EC2 {eid}")
    ...
    for vid in targets["volume"]:
        try:
            ...
            print(f"Failed to delete volume {vid}: AWS error [{code}]: {msg}")
